...
def setup_headers(sheet):
    ...
    if not headers or headers[0] == "":
        sheet.update("A1:J1", [["№", "Birja turi", "Shartnoma №", "Sana",
            "Yetkazuvchi nomi", "Tovar nomi", "O'lchov birligi",
            "Miqdori", "Shartnoma summasi", "Summa so'z bilan"]])
...
def write_to_sheet(sheet, data, row_num, serial_num):
    ...
    sheet.update(f"A{row_num}:J{row_num}", [[
        ...
        data.get("olchov_birligi", ""), miqdor, summa, soz_bilan
    ]])
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Salom! Shartnoma PDF faylini yuboring.\n\n"
